...
// ======================== TESTE: REABERTURA DE CONVERSA ========================
export async function testConversationReopen() {
  try {
    console.log(`\n🧪 [TESTE] Testando reabertura de conversa fechada...`);
    
    // 1️⃣ Buscar uma conversa ativa
    const [conv] = await db
      .select()
      .from(conversations)
      .where(eq(conversations.ativa, true))
      .limit(1);
    
    if (!conv) {
      console.log(`⚠️ Nenhuma conversa ativa encontrada para teste`);
      return { success: false, error: "Nenhuma conversa ativa" };
    }
    
    console.log(`✅ Conversa encontrada: ${conv.id} (${conv.clientId})`);
    
    // 2️⃣ FECHAR a conversa manualmente
    console.log(`🔒 Fechando conversa: ${conv.id}...`);
    await db
      .update(conversations)
      .set({ ativa: false })
      .where(eq(conversations.id, conv.id));
    
    const [closedConv] = await db
      .select()
      .from(conversations)
      .where(eq(conversations.id, conv.id));
    
    console.log(`✅ Conversa FECHADA: ativa=${closedConv.ativa}`);
    
    // 3️⃣ SIMULAR recebimento de mensagem (injetar mensagem)
    console.log(`📨 Simulando recebimento de mensagem na conversa fechada...`);
    const [newMsg] = await db
      ...
      .values({
        ...
        sender: "client",
        tipo: "texto",
        conteudo: "[TESTE] Reabrindo conversa fechada",
      })
      .returning();
    
    console.log(`✅ Mensagem injetada: ${newMsg.id}`);
    
    // 4️⃣ TESTAR se o código de reabertura funcionaria
    // Na prática, isso aconteceria no processador de mensagens do WhatsApp
    // Simulando aqui para validar a lógica:
    if (!closedConv.ativa) {
      console.log(`🔓 LÓGICA: Reabrindo conversa fechada...`);
      await db
        .update(conversations)
        .set({ ativa: true })
        .where(eq(conversations.id, conv.id));
      ...
      console.log(`✅ Conversa REABIERTA: ativa=${reopenedConv.ativa}`);
      
      return {
        success: true,
        conversationId: conv.id,
        wasActive: closedConv.ativa,
        isActive: reopenedConv.ativa,
        message: `✅ Conversa reaberta com sucesso! (ativa: ${reopenedConv.ativa})`,
      };
    }
  } catch (error) {
